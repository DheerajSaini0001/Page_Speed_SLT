...
export default function Security_Compilance({ data }) {
  ...
  return (
    ...
    >
      <h1 className={`text-3xl font-extrabold mb-6 text-center ${textColor}`}>
        Security / Compliance
      </h1>
      ...
      <div className={`w-full max-w-4xl p-6 rounded-2xl shadow-lg border-l-4 border-indigo-500 hover:scale-105 transition-transform duration-300 ${cardBg}`}>
        ...
        <div className="p-1 mt-2">
          {data.Security_or_Compliance.HTTPS.Score === 0 && (
            <h1 className="flex gap-2 text-red-600"><AlertTriangle size={20} /> HTTPS not enabled</h1>
          )}
          {data.Security_or_Compliance.HSTS.Score === 0 && (
            <h1 className="flex gap-2 text-red-600"><AlertTriangle size={20} /> HSTS missing</h1>
          )}
          {data.Security_or_Compliance.Security_Headers.Score === 0 && (
            <h1 className="flex gap-2 text-red-600"><AlertTriangle size={20} /> Security headers missing</h1>
          )}
          {data.Security_or_Compliance.Cookie_Banner_and_Consent_Mode.Score === 0 && (
            <h1 className="flex gap-2 text-red-600"><AlertTriangle size={20} /> Cookie banner / consent missing</h1>
          )}
          {data.Security_or_Compliance.Error_Pages.Score === 0 && (
            <h1 className="flex gap-2 text-red-600"><AlertTriangle size={20} /> Custom error pages not found</h1>
          )}
        </div>
      </div>
    </div>
